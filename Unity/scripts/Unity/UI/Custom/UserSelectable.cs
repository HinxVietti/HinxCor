...
namespace HinxCor.Unity.UI
{
            ...
            ISelectHandler, IDeselectHandler
    {
        ...
        // List of all the UserSelectable objects currently active in the scene
        private static List<UserSelectable> s_List = new List<UserSelectable>();
        public static List<UserSelectable> allUserSelectables { get { return s_List; } }

        // UserNavigation information.
        [FormerlySerializedAs("UserNavigation")]
        [SerializeField]
        private UserNavigation m_UserNavigation = UserNavigation.defaultUserNavigation;
        ...
        [Tooltip("Can the UserSelectable be interacted with?")]
        [SerializeField]
        ...
        public UserNavigation UserNavigation { get { return m_UserNavigation; } set { if (SetPropertyUtility.SetStruct(ref m_UserNavigation, value)) OnSetProperty(); } }
        ...
        protected SelectionState currentSelectionState
        {
            ...
            set { m_CurrentSelectionState = value; } //add 2018年12月17日14:25:58
        }
        ...
        // Find the UserSelectable object to the left of this one.
        public virtual UserSelectable FindUserSelectableOnLeft()
        {
            if (m_UserNavigation.mode == UserNavigation.Mode.Explicit)
            {
                return m_UserNavigation.selectOnLeft;
            }
            if ((m_UserNavigation.mode & UserNavigation.Mode.Horizontal) != 0)
            {
                return FindUserSelectable(transform.rotation * Vector3.left);
            }
            return null;
        }

        // Find the UserSelectable object to the right of this one.
        public virtual UserSelectable FindUserSelectableOnRight()
        {
            if (m_UserNavigation.mode == UserNavigation.Mode.Explicit)
            {
                return m_UserNavigation.selectOnRight;
            }
            if ((m_UserNavigation.mode & UserNavigation.Mode.Horizontal) != 0)
            {
                return FindUserSelectable(transform.rotation * Vector3.right);
            }
            return null;
        }

        // Find the UserSelectable object above this one
        public virtual UserSelectable FindUserSelectableOnUp()
        {
            if (m_UserNavigation.mode == UserNavigation.Mode.Explicit)
            {
                return m_UserNavigation.selectOnUp;
            }
            if ((m_UserNavigation.mode & UserNavigation.Mode.Vertical) != 0)
            {
                return FindUserSelectable(transform.rotation * Vector3.up);
            }
            return null;
        }

        // Find the UserSelectable object below this one.
        public virtual UserSelectable FindUserSelectableOnDown()
        {
            if (m_UserNavigation.mode == UserNavigation.Mode.Explicit)
            {
                return m_UserNavigation.selectOnDown;
            }
            if ((m_UserNavigation.mode & UserNavigation.Mode.Vertical) != 0)
            {
                return FindUserSelectable(transform.rotation * Vector3.down);
            }
            return null;
        }
        ...
        public virtual void OnMove(AxisEventData eventData)
        {
            switch (eventData.moveDir)
            {
                case MoveDirection.Right:
                    Navigate(eventData, FindUserSelectableOnRight());
                    break;

                case MoveDirection.Up:
                    Navigate(eventData, FindUserSelectableOnUp());
                    break;

                case MoveDirection.Left:
                    Navigate(eventData, FindUserSelectableOnLeft());
                    break;

                case MoveDirection.Down:
                    Navigate(eventData, FindUserSelectableOnDown());
                    break;
            }
        }
        ...
        public virtual void OnPointerDown(PointerEventData eventData)
        {
            ...
            // Selection tracking
            if (IsInteractable() && UserNavigation.mode != UserNavigation.Mode.None && EventSystem.current != null)
